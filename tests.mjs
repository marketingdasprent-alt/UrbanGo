...
for(const page of pages){
  ...
  for(const html of [pt,en])for(const [,href] of html.matchAll(/href="([^"]+)"/g)){if(!href.startsWith('/')||href.startsWith('//')||href.startsWith('/en/assets/'))continue;const clean=href.split('#')[0].split('?')[0];if(!clean)continue;const target=clean.endsWith('/')?`${clean.slice(1)}index.html`:clean.slice(1);await stat(`dist/${target}`).catch(()=>{throw new Error(`${page}: link local quebrado ${href}`)});}
}
